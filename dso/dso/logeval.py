...
class LogEval():
    ...
    def __init__(self, config_path):
        ...
        self.lg.info("-- LOADING LOGS START ----------------")
        self.warnings = []
        self.metrics = {}
        # Load config
        ...
        if self.summary_df is not None:
            self.lg.success("Successfully loaded summary data")
        ...
        if self.hof_df is not None:
            self.lg.success("Successfully loaded Hall of Fame data")
        # Load pareto front
        self.pf_df = self._get_log(log_type="pf")
        if self.pf_df is not None:
            self.lg.success("Successfully loaded Pareto Front data")
        # Show any warnings that occured during loading the data
        if len(self.warnings) > 0:
            self.lg.warning("*** WARNING:")
            for warning in self.warnings:
                self.lg.logging("    --> {}".format(warning))
        self.lg.info("-- LOADING LOGS END ------------------")
    ...
    def plot_results(self, results, log_type, boxplot_on=False, show_plots=False, save_plots=False):
        ...
        if save_plots:
            ...
            self.lg.logging("  Saving {} plot to {}".format(self.PLOT_HELPER[log_type]["name"], save_path))
    ...
    def analyze_log(self, show_count=5, show_hof=True, show_pf=True, show_plots=False, save_plots=False):
        ...
        self.lg.info("\n-- ANALYZING LOG START --------------")
        try:
            self.lg.logging("Task_____________{}".format(self.config["task"]["task_type"]))
            self.lg.logging("Source path______{}".format(self.save_path))
            self.lg.logging("Runs_____________{}".format(self.n_seeds))
            self.lg.logging("Max Samples/run__{}".format(self.config["training"]["n_samples"]))
            if "success_rate" in self.metrics:
                self.lg.success("Success_rate_____{}".format(self.metrics["success_rate"]))
            if len(self.warnings) > 0:
                self.lg.error("Found issues:")
                for warning in range(len(self.warnings)):
                    self.lg.warning("  {}".format(warning))
            if self.hof_df is not None and show_hof:
                hof_show_count = min(show_count, len(self.hof_df))
                self.lg.logging('Hall of Fame (Top {} of {})____'.format(hof_show_count, len(self.hof_df)))
                for i in range(hof_show_count):
                    self.lg.logging('  {:3d}: S={:03d} R={:8.6f} <-- {}'.format(
            ...
            if self.pf_df is not None and show_pf:
                self.lg.logging('Pareto Front ({} of {})____'.format(min(show_count,len(self.pf_df.index)), len(self.pf_df.index)))
                for i in range(min(show_count,len(self.pf_df.index))):
                    self.lg.logging('  {:3d}: S={:03d} R={:8.6f} C={:.2f} <-- {}'.format(
            ...
            self.lg.error("Error when analyzing!")
            for warning in self.warnings:
                self.lg.logging("    --> {}".format(warning))
        self.lg.info("-- ANALYZING LOG END ----------------")
