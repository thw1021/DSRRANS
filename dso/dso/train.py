...
def learn(sess, controller, pool, gp_controller, output_file,
          ...
          save_cache_r_min=0.9, save_freq=None, save_token_count=False, chunksize=20):
    ...
    lg.warning(f"chunksize is set to {chunksize}")
    ...
    if debug:
        ...
        def print_var_means():
            ...
            for var, val in zip(tvars, tvars_vals):
                lg.logging(var.name, "mean:", val.mean(), "var:", val.var())
    ...
    for epoch in range(n_epochs):
        ...
        # Shape of actions: np.ndarray(batch_size, max_length)
        # Shape of obs:     np.ndarray(batch_size, OBS_DIM, max_length)
        # Shape of priors:  np.ndarray(batch_size, max_length, n_choices)
        ...
        if pool is not None:
            lg.logging("Use pool to compute rewards in parallel!")
            # Filter programs that need reward computing
            programs_to_optimize = list(set([p for p in programs if "r" not in p.__dict__]))
            pool_p_dict = { p.str : p for p in pool.imap(work, programs_to_optimize, chunksize=chunksize) }
        ...
        lg.logging("Finishing computing rewards in parallel")
        ...
        if verbose and new_r_best:
            lg.logging("[{}] Training epoch {}/{}, current best R: {:.4f}".format(get_duration(start_time), epoch + 1, n_epochs, prev_r_best))
            lg.logging("\t** New best")
            p_r_best.print_stats()

        # Stop if early stopping criteria is met
        if eval_all and any(success):
            lg.logging("[{}] Early stopping criteria met; breaking early.".format(get_duration(start_time)))
            break
        ...
        if verbose and (epoch + 1) == n_epochs:
            lg.logging("[{}] Ending training after epoch {}/{}, current best R: {:.4f}".format(get_duration(start_time), epoch + 1, n_epochs, prev_r_best))

        if n_samples is not None and nevals > n_samples:
            break

    if verbose:
        lg.info("-- RUNNING EPOCHS END ---------------")
        lg.info("-- EVALUATION START ----------------")
    ...
    if verbose and priority_queue is not None:
        for i, item in enumerate(priority_queue.iter_in_order()):
            lg.logging("Priority queue entry {}:".format(i))
