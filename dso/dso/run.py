...
def train_dso(config):
    ...
    lg.info("== TRAINING SEED {} START ============".format(config["experiment"]["seed"]))
    ...
    lg.info("== TRAINING SEED {} END ==============".format(config["experiment"]["seed"]))
...
def main(config_template, runs, n_cores_task, seed, benchmark):
    ...
    if timestamp is None:
        config["experiment"]["timestamp"] = uid.moment
    ...
    for i, config in enumerate(configs):
        config["experiment"]["seed"] += pow(2, i)
    ...
    if n_cores_task > 1:
        ...
        for i, (result, summary_path) in enumerate(pool.imap_unordered(train_dso, configs)):
            if not safe_update_summary(summary_path, result):
                lg.warning("Could not update summary stats at {}".format(summary_path))
            lg.logging("Completed run {} of {} in {:.0f} s".format(i + 1, runs, result["t"]))
    else:
        for i, config in enumerate(configs):
    ...
    lg.info("== POST-PROCESS END ===================")

    if config["task"]["task_type"] == "regression":
        logdir = config["experiment"]["logdir"]
        output_log = "output" + "_" + str(uid.moment) + ".dat"
        if isinstance(config["task"]["dataset"], dict):
            case_dir = config["task"]["dataset"]["model"] + "_" + config["task"]["dataset"]["target"] + "_" + str(uid.moment)
        else:
            case_dir = Path(config["task"]["dataset"]).stem + "_" + str(uid.moment)

        shutil.move(logdir + "/" + output_log, logdir + "/" + case_dir)
