...
class GP():
    ...
    def train(self):
        ...
        if self.verbose:
            self.lg.logging("Printing {}:".format("Pareto front" if self.pareto_front else "hall of fame"))
            self.lg.logging("Fitness  |  Individual")
            for ind in self.hof:
                self.lg.logging(ind.fitness, [token.name for token in ind])
