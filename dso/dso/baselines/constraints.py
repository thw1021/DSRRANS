...
def check_inv(ind):
    ...
    for i, name in enumerate(names[:-1]):
        if name in INVERSE_TOKENS and names[i+1] == INVERSE_TOKENS[name]:
            if DEBUG:
                lg.logging("Constrained inverse:", ind)
            return True
    return False
...
def check_const(ind):
    ...
    for i, name in enumerate(names):
        if name in UNARY_TOKENS and names[i+1] == "const":
            if DEBUG:
                lg.logging("Constrained const (unary)", ind)
            return True
        if name in BINARY_TOKENS and names[i+1] == "const" and names[i+1] == "const":
            if DEBUG:
                lg.logging("Constrained const (binary)", ind)
            return True
    return False
...
def check_trig(ind):
    ...
    for i, name in enumerate(names):
        if name in TRIG_TOKENS:
            if trig_descendant:
                if DEBUG:
                    lg.logging("Constrained trig:", ind)
                return True
...
def make_check_min_len(min_length):
    ...
    def check_min_len(ind):
        ...
        if len(ind) < min_length:
            if DEBUG:
                lg.logging("Constrained min len: {} (length {})".format(ind, len(ind)))
            return True
...
def make_check_max_len(max_length):
    ...
    def check_max_len(ind):
        ...
        if len(ind) > max_length:
            if DEBUG:
                lg.logging("Constrained max len: {} (length {})".format(ind, len(ind)))
            return True
...
def make_check_num_const(max_const):
    ...
    def check_num_const(ind):
        ...
        if num_const > max_const:
            if DEBUG:
                lg.logging("Constrained max const: {} ({} consts)".format(ind, num_const))
            return True
