...
try:
    ...
    lg.logging("Failed to set cyfunc execute method, use python execute instead.")
...
class Program(object):
    ...
    def set_constants(self, consts):
        ...
        if Program.n_objects > 1:
            i_prev = 0
            for i, pos in enumerate(self.exp_trunc):
                self.traversals[i] = self.traversal[i_prev:pos+1]
                i_prev = pos + 1
    ...
    def evaluate(self):
        ...
        if "r" not in self.__dict__:
            lg.warning("Evaluating Program before computing its reward." \
    ...
    def print_stats(self):
        ...
        lg.logging("\tReward: {}".format(self.r))
        lg.logging("\tCount Off-policy: {}".format(self.off_policy_count))
        lg.logging("\tCount On-policy: {}".format(self.on_policy_count))
        lg.logging("\tOriginally on Policy: {}".format(self.originally_on_policy))
        lg.logging("\tInvalid: {}".format(self.invalid))
        lg.logging("\tTraversal: {}".format(self))

        if Program.n_objects == 1:
            lg.logging("\tExpression:")
            lg.logging("{}\n".format(indent(self.pretty()[0], '\t  ')))
        else:
            for i in range(Program.n_objects):
                lg.logging("\tExpression {}:".format(i))
