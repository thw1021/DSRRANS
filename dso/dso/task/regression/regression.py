...
class RegressionTask(HierarchicalTask):
    ...
    def __init__(self, function_set, dataset, metric="inv_nrmse",
                 ...
                 decision_tree_threshold_set=None, seed=32):
        """
        Parameters
        ----------
        ...
        if isinstance(dataset, str) and not dataset.endswith(".csv"):
            ...
            if function_set is not None and function_set != benchmark.function_set:
                self.lg.warning("Function_set provided when running benchmark "
            ...
            assert all(key in dataset for key in ("model", "cases")), "missing key word for loading turbulence dataset"

            model = dataset["model"]
            cases = dataset["cases"]
            target = dataset["target"]
            self.name = model + "_" + target
            if target == "nonLinearRS":
                self.tensor = True
                lambda_inv = loadCombinedArray(model, cases, "Lambda")
                Tensors = loadCombinedArray(model, cases, "Tensors")
                nonLinearRS = loadCombinedArray(model, cases, "nonLinearRShat")
                assert lambda_inv.shape[0] == Tensors.shape[0] == nonLinearRS.shape[0], "*** Wrong data shape ***"
                random_index = np.arange(lambda_inv.shape[0])
                np.random.shuffle(random_index)
                self.X_train = lambda_inv[random_index]
                self.tensor_basis = Tensors[random_index]
                self.y_train = nonLinearRS[random_index]
            else:
                raise ValueError("Unknown training target {}, check your configuration.".format(target))
        ...
        if dataset["invScale"]:
            lambda_sig = (1.0 - np.exp(-lambda_inv))/(1.0 + np.exp(-lambda_inv))
            lambda_sig[lambda_sig != lambda_sig] = 0.0
            if self.tensor:
                np.random.shuffle(random_index)
                self.X_train = lambda_sig[random_index]
                self.tensor_basis = Tensors[random_index]
                self.y_train = nonLinearRS[random_index]
        ...
        if self.tensor:
            ...
            if self.dimension == 2:
                self.indexs = np.array([0, 1, 4, 8])
                Program.set_n_objects(3)
                self.X_train = self.X_train[:, :2]
                self.tensor_basis = self.tensor_basis[:, :3]
            elif self.dimension == 3:
                self.indexs = np.array([0, 1, 2, 4, 5, 8])
                Program.set_n_objects(10)
            else:
                raise ValueError("dataset dimension can only be 2 or 3.")

        # If not specified, set test data equal to the training data
        if self.X_test is None:
            self.X_test = self.X_train
            if self.tensor:
                self.y_test = self.y_train.reshape(-1, 9)[:, self.indexs]
            else:
        ...
        if self.tensor:
            target_full = self.y_train.reshape(-1, 9)
            self.target = target_full[:, self.indexs]
        else:
            self.target = self.y_train
        self.metric, self.invalid_reward, self.max_reward = make_regression_metric(metric, self.target, *metric_params)
    ...
    def reward_function(self, p):
        ...
        if self.tensor:
            pre_aniso_RS = np.zeros_like(self.y_train)
            if self.dimension == 2:
                assert len(y_hat) == 3, "Number of returned expressions is {}, not 3.".format(len(y_hat))
                for i in range(self.y_train.shape[0]):
                    pre_aniso_RS[i] = y_hat[0][i] * self.tensor_basis[i, 0] \
                                    + y_hat[1][i] * self.tensor_basis[i, 1] \
                                    + y_hat[2][i] * self.tensor_basis[i, 2]
            else:
                assert len(y_hat) == 10, "Number of returned expressions is {}, not 10.".format(len(y_hat))
                for i in range(self.y_train.shape[0]):
                    pre_aniso_RS[i] = y_hat[0][i] * self.tensor_basis[i, 0] \
                                    + y_hat[1][i] * self.tensor_basis[i, 1] \
                                    + y_hat[2][i] * self.tensor_basis[i, 2] \
                                    + y_hat[3][i] * self.tensor_basis[i, 3] \
                                    + y_hat[4][i] * self.tensor_basis[i, 4] \
                                    + y_hat[5][i] * self.tensor_basis[i, 5] \
                                    + y_hat[6][i] * self.tensor_basis[i, 6] \
                                    + y_hat[7][i] * self.tensor_basis[i, 7] \
                                    + y_hat[8][i] * self.tensor_basis[i, 8] \
                                    + y_hat[9][i] * self.tensor_basis[i, 9]
            y_hat = pre_aniso_RS.reshape(-1, 9)
            predict = y_hat[:, self.indexs]
        else:
            predict = y_hat
        ...
        r = self.metric(self.target, predict)
    ...
    def evaluate(self, p):
            ...
            if self.dimension == 2:
                ...
                for i in range(self.y_train.shape[0]):
                    pre_aniso_RS[i] = y_hat[0][i] * self.tensor_basis[i, 0] \
                                    ...
                                    + y_hat[9][i] * self.tensor_basis[i, 9]
            y_hat = pre_aniso_RS.reshape(-1, 9)
            test_predict = y_hat[:, self.indexs]
        else:
            test_predict = y_hat
        ...
        if p.invalid:
            ...
            nmse_test = np.mean((self.y_test - test_predict) ** 2) / self.var_y_test

            # NMSE on noiseless test data (used to determine recovery)
            nmse_test_noiseless = np.mean((self.y_test_noiseless - test_predict) ** 2) / self.var_y_test_noiseless
        ...
        if self.metric_test is not None:
            if p.invalid:
                ...
                m_test = self.metric_test(self.y_test, test_predict)
                m_test_noiseless = self.metric_test(self.y_test_noiseless, test_predict)
...
def make_regression_metric(name, y_train, *args):
    ...
    all_metrics = {
        ...
        # Range: [0, 1]
        "pearson" :     (lambda y, y_hat : np.array(scipy.stats.pearsonr(y.flatten(), y_hat.flatten())[0]),
                        0),

        # Spearman correlation coefficient
        # Range: [0, 1]
        "spearman" :    (lambda y, y_hat : np.array(scipy.stats.spearmanr(y.flatten(), y_hat.flatten())[0]),
                        0)
    }
...
data_dir  = '../../../../turbulenceCases/training/dataset/'

def loadCombinedArray(model, cases, field):
    data = np.concatenate([np.load(data_dir+model+'/'+case+'_'+field + '.npy') for case in cases])
    return data