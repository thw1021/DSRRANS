    ...
    def save_results(self, positional_entropy, top_samples_per_batch, r_history, pool, n_epochs, n_samples):
        ...
        if self.hof is not None and self.hof > 0:
            ...
            if self.hof_output_file is not None:
                self.lg.logging("Saving Hall of Fame to {}".format(self.hof_output_file))
                df.to_csv(self.hof_output_file, header=True, index=False)

            #save cache
            if self.save_cache and Program.cache:
                self.lg.logging("Saving cache to {}".format(self.cache_output_file))
            ...
            if self.save_pareto_front:
                ...
                if self.pf_output_file is not None:
                    self.lg.logging("Saving Pareto Front to {}".format(self.pf_output_file))
            ...
            if n_invalid > 0:
                self.lg.logging("Invalid expressions: {} of {} ({:.1%}).".format(n_invalid, n_samples,
                                                                       n_invalid / n_samples))
                self.lg.logging("Error type counts:")
                for error_type, count in error_types.items():
                    self.lg.logging("  {}: {} ({:.1%})".format(error_type, count, count / n_invalid))
                    result["error_"+str(error_type)] = count
                self.lg.logging("Error node counts:")
                for error_node, count in error_nodes.items():
                    self.lg.logging("  {}: {} ({:.1%})".format(error_node, count, count / n_invalid))
