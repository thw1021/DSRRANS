...
def make_prior(library, config_prior):
    ...
    lg.info("-- BUILDING PRIOR START -------------")
    lg.warning("\n"+"\n".join([message for message in warn_messages]))
    lg.logging(joint_prior.describe())
    lg.info("-- BUILDING PRIOR END ---------------\n")
...
class JointPrior():
    ...
    def report_constraint_counts(self):
        if not self.do_count:
            return
        self.lg.logging("Constraint counts per prior:")
        for i, count in zip(self.constraint_indices, self.constraint_counts):
            self.lg.logging("{}: {} ({:%})".format(self.names[i], count, count / self.total_tokens))
        self.lg.logging("All constraints: {} ({:%})".format(self.total_constraints, self.total_constraints / self.total_tokens))
...
class LengthConstraint(Constraint):
    ...
    def describe(self):
        ...
        if self.min is not None:
            message.append("\n{}: Sequences have minimum length {}.".format(self.__class__.__name__, self.min))
        if self.max is not None:
